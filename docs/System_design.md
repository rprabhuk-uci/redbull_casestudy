...
| Brand names appearing in ID columns | Unmatched/stray quote characters in free-text fields (e.g. `item_desc`) caused cascading column shifts across rows | `TRY_TO_DECIMAL` validation on key ID columns → quarantine table, not silently dropped |
